...
The GitHub Pages workflow reconstructs the verified v4.1.1 base, applies the compact evidence-driven v4.2.0 line patch, verifies the resulting v4.2.0 SHA-256 and byte count, and only then deploys it as `index.html`.
...
- `release/v4.2.0/SHA256SUMS` — canonical resulting v4.2.0 integrity hash
...
The deployment fails rather than publishing if either the frozen v4.1.1 base hash or the reconstructed v4.2.0 SHA-256/byte count differs from the expected values. v4.1.1 remains preserved for regression comparison.
